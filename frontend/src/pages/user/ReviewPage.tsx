...
const RATING_CONFIG = [
  ...
  { value: 3, label: 'Ingat', key: '3', color: 'bg-brand-50 text-brand-600 border-brand-200 hover:bg-brand-100 hover:border-brand-300' },
  ...
  if (loading) {
    return (
      <div className="flex flex-col items-center justify-center min-h-[60vh] gap-3">
        <div className="w-10 h-10 rounded-2xl bg-brand-50 border border-brand-100 flex items-center justify-center animate-glow-pulse">
          ...
          <div className="w-full card p-4 mb-6">
            ...
            <div className="grid grid-cols-4 gap-2 text-center">
              ...
              <div className="bg-brand-50 p-2 rounded-xl border border-brand-100">
                <p className="text-base font-semibold text-brand-600">{ratingCounts[3] || 0}</p>
                <p className="text-[10px] text-brand-700 font-medium">Ingat</p>
              </div>
  ...
  return (
    <div className="max-w-xl mx-auto space-y-6 animate-fade-in">
      ...
      <div className="w-full bg-slate-100 rounded-full h-1.5 overflow-hidden">
        <div
          className="h-full bg-gradient-to-r from-brand-500 to-violet-500 rounded-full transition-all duration-300 ease-out"
      ...
      >
        ...
        >
          {/* Front */}
          <div className="absolute inset-0 backface-hidden card p-8 flex flex-col justify-between hover:shadow-card-hover transition-shadow">
            <div className="flex items-center justify-between">
              <span className="text-[11px] uppercase tracking-widest text-slate-400 font-semibold flex items-center gap-1">
                <BookOpen className="w-3.5 h-3.5 text-brand-500" />
                Pertanyaan
              </span>
              <span className="text-[10px] uppercase font-mono px-2 py-0.5 rounded bg-slate-100 text-slate-600 font-medium border border-slate-200/60">
          ...
          {/* Back */}
          <div className="absolute inset-0 backface-hidden rotate-y-180 bg-white border border-brand-200 rounded-2xl p-8 flex flex-col justify-between shadow-card">
            <div className="flex items-center justify-between">
              <span className="text-[11px] uppercase tracking-widest text-brand-600 font-semibold flex items-center gap-1">
                <Sparkles className="w-3.5 h-3.5 text-brand-500" />
                Jawaban
              </span>
              <span className="text-[10px] uppercase font-mono px-2 py-0.5 rounded bg-brand-50 text-brand-600 font-medium border border-brand-100">
                {currentCard.card_type || 'Concept'}
              </span>
            </div>
      ...
      {/* FSRS metadata */}
      <div className="flex items-center justify-between card px-4 py-2 text-xs text-slate-500">
      ...
      {isFlipped ? (
          ...
          <div className="grid grid-cols-4 gap-2.5">
            {RATING_CONFIG.map((r) => (
              <button
                ...
                className={`py-3 rounded-xl border text-xs font-medium transition-all duration-200 disabled:opacity-50 flex flex-col items-center gap-1 shadow-xs active:scale-[0.96] ${r.color}`}
              >
                ...
                <kbd className="text-[10px] font-mono opacity-60 bg-black/5 px-1.5 py-0.5 rounded">
                  [{r.key}]
                </kbd>
              </button>
            ))}
        ...
        <button
          ...
          className="w-full py-3 rounded-xl bg-slate-900 hover:bg-slate-800 text-white text-xs font-medium transition-colors shadow-sm flex items-center justify-center gap-2 active:scale-[0.99]"
        >
